    ...
    def get_html(self, url, method='get', body=None, headers=config.HEADERS,allow_redirects=True,timeout=20):
        ...
        try:
            if config.COOKIES_SWITCH:
                ...
                if config.PROXY_SWITCH:
                    if method == 'get':
                        response = requests.get(url, headers=headers, proxies=proxies, timeout=timeout,allow_redirects=allow_redirects)
                    elif method == 'post':
                        response = requests.post(url, headers=headers, data=body, proxies=proxies, timeout=timeout,allow_redirects=allow_redirects)
                else:
                    if method == 'get':
                        response = requests.get(url, headers=headers, timeout=timeout,allow_redirects=allow_redirects)
                    elif method == 'post':
                        response = requests.post(url, headers=headers, data=body, timeout=timeout,allow_redirects=allow_redirects)
            return response
