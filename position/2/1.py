...
id_list = []
def start():
    start_url = 'http://yssxy.bibibi.net/module/getonlines?start={pageToken}&count=16&k=&professionals=&recruit_type='
        ...
        try:
            ...
            for data in json_obj['data']:
                try:
                    ...
                    detail_url = 'http://yssxy.bibibi.net/detail/online?id='+str(id)
                    detail_response = requests.get(detail_url,headers=headers)
                    # print(detail_response.text)
                    comAddress = re.search('<p class="ci-text">.*?地址：(.*?)</p>',detail_response.text,re.S)
                    if comAddress:
                        comAddress = comAddress.group(1)
                    else:
                        comAddress = ''

                    url2 = 'http://yssxy.bibibi.net/detail/getonlinedetail?online_id='+str(id)
                    res2 = requests.get(url2,headers=headers)
                    jsonObj2 = json.loads(res2.text)

                    # print(jsonObj2['data']['content'])
                    html2 = HTML(jsonObj2['data']['content'])
                    zhizeList = html2.xpath('//p//text()')
                    zhize = ''.join(zhizeList).replace('\n', '').replace('\r', '').replace('\t', ' ').strip()

                    comName = data['company_name']
                    positionName = data['job_recruitment']
                    jobType = data['recruit_type']


                    price = jsonObj2['data']['jobs'][0]['salary']
