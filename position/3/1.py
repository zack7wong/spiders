...
id_list = []
def start():
    start_url = 'http://glut.doerjob.com/nologin/school!positionIndex.htm'
    response = requests.get(url=start_url,headers=headers)
    # print(response.text)
    html = HTML(response.text)
    ul_list = html.xpath('//ul[@id="doubleJobFairPositionListId"]/li')
    url_list = html.xpath('//ul[@id="doubleJobFairPositionListId"]/li/div[1]/div[1]/a/@href')
    print(len(ul_list))
    for li,url in zip(ul_list,url_list):
        link = 'http://glut.doerjob.com'+url
        positionName = li.xpath('string(./div[1]/div[1]/a/text())')
        comName = li.xpath('string(./div[2]/div[1]/text())')
        comAddress = li.xpath('string(./div[2]/div[3]/text())')
        jobType = ''
        zhize = li.xpath('string(./div[3]/div[5]/span/text())').replace('\n', '').replace('\r', '').replace('\t', ' ').strip()

        response = requests.get(link)
        price = re.search('"rmb-month"><b>(.*?)</b></span>',response.text)
        if price:
            price = price.group(1)
        else:
            price = ''
